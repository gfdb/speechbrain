...
Google Speech Commands v0.02 Dataset with step-based LR scheduling.

This variant uses LinearStepScheduler (per-optimizer-step) instead of
the epoch-based LinearScheduler used in train.py.

To run this recipe, use the following command:
> python train_views.py hparams/top9_views.yaml
...
class SpeakerBrain(sb.core.Brain):
    ...
    def on_fit_batch_end(self, batch, outputs, loss, should_step):
        """At the end of the optimizer step, apply step-based LR annealing."""
        if should_step:
        ...
        if stage == sb.Stage.VALID:
            self.hparams.train_logger.log_stats(
                stats_meta={
                    ...
                    "steps": self.hparams.lr_annealing.n_steps,
                },
