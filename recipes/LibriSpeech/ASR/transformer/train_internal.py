    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            if hasattr(self.hparams, 'dirty_mse') and self.hparams.dirty_mse:
                ...
                dirty_loss_mse += F.mse_loss(dirty1, dirty2, reduction="mean")
                dirty_loss_mse += F.mse_loss(dirty1, dirty3, reduction="mean")
                dirty_loss_mse += F.mse_loss(dirty2, dirty3, reduction="mean")
                dirty_loss_mse += F.mse_loss(dirty2, dirty1, reduction="mean")
                dirty_loss_mse += F.mse_loss(dirty3, dirty1, reduction="mean")
                dirty_loss_mse += F.mse_loss(dirty3, dirty2, reduction="mean")

                dirty_loss_mse = dirty_loss_mse / 6
        ...
        return p_ctc, p_seq, wav_lens, hyps, clean_kl_loss, dirty_kl_loss, clean_internal_loss_mse, dirty_loss_mse

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (CTC+NLL) given predictions and targets."""

        (p_ctc, p_seq, wav_lens, hyps, clean_kl_loss, dirty_kl_loss, clean_internal_loss_mse, dirty_loss_mse) = predictions
        ...
        if stage == sb.Stage.TRAIN:
            if hasattr(self.hparams, "usa_speed") and self.hparams.usa_speed:
                ...
                ) + (self.hparams.dirty_mse_weight) * dirty_loss_mse 
                print('ctc:', loss_ctc)
                print('seq:', loss_seq)
                print('kl clean:', clean_kl_loss)
                print('kl dirty:', dirty_kl_loss)
                print('mse clean:', clean_internal_loss_mse)
                print('mse weight:', self.hparams.clean_mse_weight)
                print('dirty_mse:', dirty_loss_mse)
                print('dirty_mse_weight:', self.hparams.dirty_mse_weight)
        else:
...
def dataio_prepare(hparams):
    ...
    sampled_ids = random.sample(train_data.data_ids, 8)
    ...
    sampled_ids = random.sample(valid_data.data_ids, 8)
    valid_data = FilteredSortedDynamicItemDataset(valid_data, sampled_ids)
    # test is separate
    test_datasets = {}
